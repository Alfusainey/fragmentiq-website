"use client";

import { useState } from "react";
import Link from "next/link";
import { ChevronDown } from "lucide-react";

const useCases = [
  {
    label: "AI-Generated Code Integration",
    href: "/use-cases/ai",
  },
  {
    label: "Internal Project Copying",
    href: "/use-cases/engineering",
  },
];

export function Footer() {
  const [isUseCasesOpen, setIsUseCasesOpen] = useState(false);

  return (
    <footer className="bg-white/50 px-8 pt-24 pb-16 md:pt-32 md:pb-20 lg:pt-40">
      <div className="mx-auto max-w-screen-2xl">
        {/* Main footer content */}
        <div className="flex flex-col items-center gap-12 md:flex-row md:justify-between md:items-center">
          {/* Logo */}
          <div className="flex-shrink-0">
            <Link href="/" className="text-2xl font-serif italic text-foreground">
              Distributed Code Intelligence
            </Link>
          </div>

          {/* Navigation */}
          <nav className="flex items-center gap-12">
            <Link 
              href="/" 
              className="text-base font-medium text-foreground hover:opacity-70 transition-opacity"
            >
              Home
            </Link>
            
            {/* Use Cases Dropdown */}
            {/* <div
              className="relative"
              onMouseEnter={() => setIsUseCasesOpen(true)}
              onMouseLeave={() => setIsUseCasesOpen(false)}
            >
              <button
                className="flex items-center gap-1 text-base font-medium text-foreground hover:opacity-70 transition-opacity"
                onClick={() => setIsUseCasesOpen(!isUseCasesOpen)}
              >
                Use Cases
                <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${isUseCasesOpen ? 'rotate-180' : ''}`} />
              </button>
              
              {// Dropdown Menu }
              <div
                className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 bg-white rounded-lg border border-border shadow-lg overflow-hidden transition-all duration-200 origin-bottom ${
                  isUseCasesOpen
                    ? 'opacity-100 scale-100 translate-y-0'
                    : 'opacity-0 scale-95 translate-y-2 pointer-events-none'
                }`}
              >
                {useCases.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="block px-4 py-3 text-sm text-foreground hover:bg-muted transition-colors"
                    onClick={() => setIsUseCasesOpen(false)}
                  >
                    {item.label}
                  </Link>
                ))}
              </div>
            </div> */}
          </nav>

          {/* Social icons */}
          <div className="flex items-center gap-6">
            {/* X / Twitter */}
            <a
              href="#"
              className="text-foreground hover:opacity-70 transition-opacity"
              aria-label="X (Twitter)"
            >
              <svg
                className="w-5 h-5"
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
              >
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>
            {/* LinkedIn */}
            <a
              href="#"
              className="text-foreground hover:opacity-70 transition-opacity"
              aria-label="LinkedIn"
            >
              <svg
                className="w-5 h-5"
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
              >
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
              </svg>
            </a>
            {/* YouTube */}
            <a
              href="#"
              className="text-foreground hover:opacity-70 transition-opacity"
              aria-label="YouTube"
            >
              <svg
                className="w-5 h-5"
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
              >
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
              </svg>
            </a>
          </div>
        </div>

        {/* Divider */}
        <div className="mt-16 mb-12 border-t border-foreground/20" />

        {/* Copyright */}
        <p className="text-center text-base text-muted-foreground">
          © 2026 Understanding recurring code fragments across ecosystems
        </p>
      </div>
    </footer>
  );
}
